...
int main() {
    ...
    CacheConfig config{
        ...
        .verbose = true          // Silence -> no printing
    };
    ...
    // Showcase: drive a synthetic workload, then use the new debug + profiling
    // tooling (cache dump + stats report) instead of the raw memory matrix.
    Test("Showcase: replay a looping workload then profile the cache");
    RunTrace(memory, GenRandom(0x2000, /*span=*/512, /*loops=*/4, /*stride=*/4));
    memory.DumpCache();
    memory.PrintStats();
